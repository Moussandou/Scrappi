        ...
        <div className="fixed right-4 md:right-8 top-24 bottom-24 w-64 pointer-events-auto z-[40] flex flex-col transition-all duration-300">
            <div className={`bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-soft border border-black/5 flex flex-col transition-all duration-500 ease-in-out overflow-hidden ${isCollapsed ? 'h-14 opacity-60' : 'h-full max-h-[60vh] md:max-h-full opacity-100'}`}>
                ...
                <div className={`flex-1 overflow-y-auto custom-scrollbar px-4 md:px-5 pb-4 md:pb-5 space-y-6 md:space-y-8 transition-all duration-300 ${isCollapsed ? 'opacity-0 invisible pointer-events-none hidden' : 'opacity-100 visible'}`}>
                    ...
                    {/* Storage Settings */}
                    <div className="flex flex-col gap-3">
                        <SectionHeader id="storage" label="Stockage Local" expanded={expandedSections.storage} onToggle={toggleSection} />
                        {expandedSections.storage && (
                            <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
                                <p className="text-[10px] leading-relaxed text-ink/70 px-1">
                                    Vos images et vidéos sont stockées <b>uniquement</b> sur votre ordinateur pour garantir un stockage infini et sécurisé.
                                </p>
                                ...
                                {storageMode.isLocalSupported ? (
                                    ...
                                    <p className="text-[9px] text-red-500 bg-red-50 p-2 rounded-xl text-center">
                                        Navigateur non compatible (utilisez Chrome ou Edge). Les images ne pourront pas être sauvegardées.
                                    </p>
                                )}
                            </div>
                        )}
